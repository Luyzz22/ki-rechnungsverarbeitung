...
def get_user_info(user_id):
    ...
    if not user_id:
        return {"id": 0, "email": "", "name": "User", "is_admin": False}
    try:
        import sqlite3
        ...
        conn.close()
        if row:
            return {"id": row["id"], "email": row["email"], "name": row["name"] or "User", "is_admin": bool(row["is_admin"])}
    except Exception:
        pass
    return {"id": user_id, "email": "", "name": "User", "is_admin": False}
...
# ============================================================
# PDF-VIEWER ROUTE - Enterprise Document Access
# ============================================================

@router.get("/api/invoice/pdf/{invoice_id}")
async def view_invoice_pdf(request: Request, invoice_id: int):
    """Sichere PDF-Anzeige für Rechnungen mit User-Validierung"""
    user_id = request.session.get("user_id")
    if not user_id:
    ...
    cursor.execute("""
        SELECT i.id, i.job_id, i.rechnungsnummer, i.rechnungsaussteller,
               j.upload_path
        FROM invoices i
    ...
    if not upload_path or not os.path.exists(upload_path):
        return JSONResponse({"error": "PDF path not found"}, status_code=404)
    
    pdf_files = glob.glob(os.path.join(upload_path, "*.pdf")) + glob.glob(os.path.join(upload_path, "*.PDF"))
    if not pdf_files:
        return JSONResponse({"error": "No PDF file found"}, status_code=404)
